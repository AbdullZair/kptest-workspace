...
export const PageLoader = memo(
  forwardRef<HTMLDivElement, PageLoaderProps>(
    ({ size = 'md', text = 'Loading...', overlay = false, className, ...props }, ref) => {
      ...
      return (
        <div ref={ref} className={twMerge(baseStyles)} {...props}>
          <svg
            className={clsx('animate-spin', 'text-primary-600', sizeStyles[size].spinner)}
          ...
          {text ? <p className={clsx('mt-4 font-medium', sizeStyles[size].text)}>{text}</p> : null}
        </div>
      )
    }
  )
